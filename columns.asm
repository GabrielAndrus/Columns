...
##############################################################################
# Commands for our game:
  # a, s, and d for movement. 
  # w rotates the columns.
  # r will flip the column horizontally
...
create_new_column:
    ...
    jal toggle_draw_mode # switch shape every new piece
    la $t0, colorsArray
    ...
    addi $t3, $t3, 1 # works for all shapes -- do not touch.

game_loop:

    # 1a. Check if key has been pressed
    lw $t8, 0($t1)
    jal remove_row # constantly erases -- change if removing shape
    ...
    beq $t8, 0x114, respond_to_r    # 'r'
    
    
    # 2a. Check for collisions.

# Draws the column/shape to the side of the grid -- update given new shape
side_draw:
    push($ra)
    addi $sp, $sp, -4   #save variable that is temporarily used
    sw $t0, 0($sp)
    ...
    jal draw_shape # will have to update depending on the current shape --> draw_shape function.
...
collision:  # When collision happens, draws thes column/shape where it was
    jal draw_shape # should still work for different shapes, but we will see
    li $v0, 32
    li $a0, 100
    syscall
...
no_input:   
    ...
    bne $v0, $t8, draw_column_and_create # draw shape (from random shape)
    
    #li $t4, 
    addi $t3, $t3, 1 # configure so column is falling (last variable is rate of fall -- max = 3)
    #addi $a3, $a3, 1

  jal draw_shape # this line decides the shape of the block.
...
# when q pressed, exit program
respond_to_q:
  li $v0, 10
  syscall

# rotate the shape (vertical -> horizontal, vice versa)
respond_to_r:
  #lw $a0, draw_mode 
  #xori $a0, $a0, 1
  #sw $a0, draw_mode # change the modeca
  #sw $a0, current_shape # save current shape (1 or 0) into current_shape variable
...
draw_column_and_create: # create a similar draw_shape and create function
...
draw_pixel:
    ...
    lw $t0, ADDR_DSPL # necessary for game_over function
    li $t6, 32
    mult $t3, $t6
